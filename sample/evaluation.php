...
?>
    ...
    <div class="container">
    <form id="form1" name="form1" method="post" action="evaluation.php"  class="form-inline">
...
//count object
$sql_count="SELECT DISTINCT cmid FROM mdl_block_point_view WHERE courseid='3' ORDER By `cmid`";
$statement_count = $db->query($sql_count);
$show_count = mysqli_num_rows($statement_count);
echo "show=".$show_count."      ";
//count finish



$sql_post="SELECT A.cmid,A.vote,B.name 
FROM mdl_block_point_view A,mdl_modules B 
WHERE A.courseid='3' and A.cmid=B.id
ORDER BY A.cmid";
$i=0;
...
foreach($statement_post as $row){
	$i++;
	// echo $i."<br>";
    echo  $row['cmid']."  ".$row['vote']."  ". $row['name']."<br>";
